...
class ObjectDetector(Node):
    ...
    def image_callback(self,msg):
      ...
      sign_delay = 0
      light_delay = 0
      # self.get_logger().info(f"debug: Encoding: {msg.encoding}, W: {msg.width}, H: {msg.height}, Step: {msg.step}, DataLen: {len(msg.data)}")
      try:
          cv_image = self.bridge.imgmsg_to_cv2(msg, desired_encoding='bgr8')
      except Exception as e:
          self.get_logger().warn(f"Failed to convert image to bgr8: {e}. Trying passthrough.")
          try:
              cv_image = self.bridge.imgmsg_to_cv2(msg, desired_encoding='passthrough')
          except Exception as e2:
              self.get_logger().error(f"Failed to convert image even with passthrough: {e2}")
              return
      
      if cv_image is None or cv_image.size == 0:
          # self.get_logger().debug("Received empty image or conversion failed")
          return
