...
`
      ...
      self.publisher = self.create_publisher(MotorCommands,'/qcar2_motor_speed_cmd', 1)
      self.cyclic = False
    ...
    def path_planner(self):
        ...
        # publishing commands
        self.get_logger().info(f"Debug: execute={self.path_execute_flag}, motion={self.motion_flag}, enable={enable}, speed={speed_command}")
        self.nav_command(enable, speed_command)
        self.path_status()

    def nav_command(self,enable, speed_command):
      self.get_logger().info(f"Publishing cmd: enable={enable}, speed={speed_command}")
      QCarCommands = MotorCommands()
      QCarCommands.motor_names = ['steering_angle', 'motor_throttle']
      steering = enable*self.current_steering
      throttle = enable*np.clip(speed_command*np.power(np.cos(self.current_steering),2),0.05,0.7)
      QCarCommands.values = [float(steering), float(throttle)]
